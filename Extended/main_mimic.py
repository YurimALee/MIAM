...
from Upload.models_mimic import *
import torch
...
# Load Kfold dataset
data_dir ='/home/yrlee/mimic/irregular/'# Data direction
...
kfold_label_phe = np.load(open(data_dir + 'label_phenotyping.p', 'rb'), mmap_mode='r', allow_pickle=True)
...
lambda_2 = 0
lambda_3 = 10
print('focal(y):', str(lambda_1), ', rmse(x):', str(lambda_2), ', mse(x):', str(lambda_3))
...
writelog(f, 'cls:'+ str(lambda_1) + ', phe:' + str(lambda_2) +', imp:'+ str(lambda_3))
writelog(f, '---------------')
...
def train(epoch, train_loader):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        loss_phe = criterion_focal(model, los_out, y1)
        loss_cls = criterion_focal(model, y_hat, y)
        loss_imp = criterion_mse(out, x)
        loss = lambda_1*loss_cls + lambda_2*loss_phe + lambda_3*loss_imp
...
def test(phase, epoch, test_loader):
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        loss_phe = criterion_focal(model, los_out, y1)
        loss_cls = criterion_focal(model, y_hat, y)
        loss = lambda_1*loss_cls + lambda_2*loss_phe


        test_loss += loss.item()
        n_batches += 1

        y_score = y_hat #output
        ...
        y_prd = np.hstack([y_prd, y_pr.flatten()])
        y_scs = np.hstack([y_scs, y_sc.flatten()])
    ...
    # auc, auprc, acc, balacc, sen, spec, prec, recall = calculate_performances(y_gts, y_scores, y_preds)
    auc, auprc, acc, balacc, sen, spec, prec, recall = calculate_performance(y_gt, y_scs, y_prd)
    ...
    return auc, auprc, acc, balacc, sen, spec, prec, recall#rmse, mae,
...
# for k in switch(fold_num):
    ...
    # Get Dataset
    Data_dir = '/DataCommon/yrlee/mimic/'

    train_data_zero = np.load(Data_dir + str(k) + '_train_data_zero.npy')
    train_data, train_data_org, train_rmask  = random_mask(train_data_zero)
    train_mask = np.load(Data_dir + str(k) + '_train_mask.npy')
    train_label = np.load(Data_dir + str(k) + '_train_label.npy')
    train_label2 = kfold_label_phe[k][0]
    train_time = np.load(Data_dir + str(k) + '_train_time.npy')

    valid_data_zero = np.load(Data_dir + str(k) + '_valid_data_zero.npy')
    valid_mask = np.load(Data_dir + str(k) + '_valid_mask.npy')
    valid_label = np.load(Data_dir + str(k) + '_valid_label.npy')
    valid_label2 = kfold_label_phe[k][1]
    valid_time = np.load(Data_dir + str(k) + '_valid_time.npy')

    test_data_zero = np.load(Data_dir + str(k) + '_test_data_zero.npy')
    test_mask = np.load(Data_dir + str(k) + '_test_mask.npy')
    test_label = np.load(Data_dir + str(k) + '_test_label.npy')
    test_label2 = kfold_label_phe[k][2]
    test_time = np.load(Data_dir + str(k) + '_test_time.npy')

    # Define Loaders
    train_loader = msk_sample_loaders('train', k, train_data_org, train_mask, train_data, train_mask, train_label, train_label2, train_time, batch_size, ZeroImpute=True)
    valid_loader = sample_loaders('valid', k, valid_data_zero, valid_mask, valid_label, valid_label2, valid_time, batch_size, ZeroImpute=True)
    test_loader = sample_loaders('test', k, test_data_zero, test_mask, test_label, test_label2, test_time, batch_size, ZeroImpute=True)
...
# Define Model & Optimizer
    ...
    for epoch in range(n_epochs):
        ...
        # rmse, mae, auc, auprc, acc, balacc, sen, spec, prec, recall = test('valid', epoch, valid_loader)#
        ...
        #     writelog(f, 'Lowest validation MSE is found! Validation MSE: ' + str(rmse))
        ...
        writelog(f, 'Test')
        # rmse, mae, auc, auprc, acc, balacc, sen, spec, prec, recall = test('test', epoch, test_loader)
        auc, auprc, acc, balacc, sen, spec, prec, recall = test('test', epoch, test_loader)#rmse, mae,
        scheduler.step()

    # Load Best Validation AUC
    miam_best_model = torch.load(dir + 'model/' + str(k) + '/' + str(best_epoch) + '_self_attention.pt')
...
# mean_rmse = round(np.mean(kfold_rmse), 5)
# std_rmse = round(np.std(kfold_mae), 5)
