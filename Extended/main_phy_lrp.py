...
def test(phase, epoch, test_loader):
    ...
    for tag, value in info.items():
        summary = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=tag, simple_value=value)])#tf.Summary(value=[tf.Summary.Value(tag=tag, simple_value=value)])
        if phase == 'valid':
...
# Get dataset 1
    ...
    valid_data[val_miss_idx] = 0#np.nan
    ...
    test_loader = msk_sample_loader('test', k, test_data, test_mask, test_label, test_label2, test_time, batch_size, ZeroImpute=True)
